...
export function watch() {
  gulp.watch(config.src + config.styles + '**/*.scss', gulp.series(styles));
  gulp.watch(config.src + config.views + '**/*.scss', gulp.series(styles));
  gulp.watch(config.src + config.scripts + '**/*.js', gulp.series(mainScripts, vendorScripts));
}

const build = gulp.series(mainScripts, vendorScripts, svg, bitmap, sassGenerateContents, styles, copyFonts);
